...
- Specs at PRD 1.10 / API 1.12 / Tech 1.10 / Data Model 1.7 / Plan 1.4, constitution 1.2. Eighteen deltas are archived under `changes/_archive/` and none is pending. The last of them, `2026-09-bootstrap-sweeper` (ratified 2026-09-20), puts a shell's bootstrap files in the daemon's runtime directory and has it sweep the previous run's at start behind the instance lock; its implementation, **T-F0-19, is not written**. The last of them, `2026-09-restore-semantics`, stops a restart and `layout.apply` running a stored command, gives focus and `command_pending` their columns, moves `pane_history` to migration `0004_restore`, and specifies `$XDG_CONFIG_HOME/umbral/config.toml`.
