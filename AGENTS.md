...
- Specs at PRD 1.7 / API 1.8 / Tech 1.8 / Data Model 1.6 / Plan 1.4, constitution 1.2. Thirteen deltas are archived under `changes/_archive/`. **One is pending ratification:** `changes/2026-09-notification-sequencing/`, which pins where a notification's `seq` lives, what it counts and which `seq` it is not; it is already applied to `specs/`.
