...
- Specs at PRD 1.7 / API 1.8 / Tech 1.8 / Data Model 1.6 / Plan 1.4, constitution 1.2. Thirteen deltas are archived under `changes/_archive/` and none is pending; the last of them, `2026-09-pane-fields-and-api-imports`, adds `command` and `env` to API §4's `Pane` and admits into Tech Design §5.2 the `domain` imports `api` cannot avoid.
