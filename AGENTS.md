...
- Specs at PRD 1.8 / API 1.11 / Tech 1.8 / Data Model 1.6 / Plan 1.4, constitution 1.2. Sixteen deltas are archived under `changes/_archive/` and **none is pending**: `2026-09-schema-and-degradation-corrections` was ratified on 2026-09-20 and scopes §9's registration rule to what a build *implements*, brings §9's capability bullet into line with §2, makes `limit`/`cursor`/`include` optional, states the JSON Schema dialect `api.schema` emits, and writes down the request shapes of seventeen methods that had none.
