...
- **State:** `[~]`, not `[x]`. The code is written and the whole gate is green, but the Done line has two halves and only one is met: every row of `docs/qa/f0-tui.md` is still `☐`. That half is a person's job — raw mode, a real keyboard, a real font and a week of use are what it covers, and none of it can be asserted from here. The task closes when the checklist's Result table is filled in.
...
- **State:** `[~]`, not `[x]`. The first half of the Done line is the unmet one: `.github/workflows/perf.yml` has never executed. Every figure quoted below was measured on the ThinkPad, and PRD §7 names the CI runner — GitHub `ubuntu-latest`, 4 vCPU — as the reference hardware for the automated gate, precisely so its thresholds are judged there. The scripts the job runs were driven locally in both directions; the job around them was not. It closes on the first push or pull request that turns it green, or, if the runner cannot reach a number, on the adjustment PRD §7 prescribes — gate threshold moved, laptop figure kept as the product target, both written down.
